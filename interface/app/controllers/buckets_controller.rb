class BucketsController < ApplicationController
  ...
  # GET /buckets.json
  def index
    @buckets = current_user.company.buckets.all
  end
  ...
  # GET /buckets/new
  def new
    @bucket = current_user.company.buckets.new
  end
  ...
  # POST /buckets.json
  def create
    @bucket = current_user.company.buckets.new(bucket_params)
  ...
  private
    ...
    def set_bucket
      @bucket = current_user.company.buckets.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def bucket_params
      params.require(:bucket).permit(:name, :company_id)
    end
end
